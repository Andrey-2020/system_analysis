...
* Уменьшение затрат времини времени на контроль подчиненных
...
* **SH-2**: **Заказчик** (доступность, производительность, масштабируемость, надежность)
...
* **UC-1**: **Подача и рассмотрение заявки на стажировку**:
    ...
    - руководитель стажировки использует свой личный кабинет (ЛК) для оценки информации о кандидате, выполненного домашннего задания и прития решения в рамках системы – нажимает кнопку “подходит” кандидат или “отклонить” (SH-4);
    - администратор в ЛК получает отдельный список, где указаны статусы одобрено или отклонено для каждого кандидата. Если кандидат отклонен, администратор может вручную (уже не в рамках системы) связаться с ним путем написания письма или звонка, где проинформирует о том, что он не принят на стажировку. В случае одобрения кандидата, администратор нажимает кнопку “ ”. Дальше возвращается статус. Если статус “проверка пройдена успешно”, администратор нажимает кнопку “зарегистрировать”, и стажер регистрируется в системе. Если статус “проверка не пройдена”, стажера не регистрируют (SH-1)

* **UC-2**: **Регистрация стажера**:
    - Администратор имеет функционал регистрации профиля стажера и отправки информации о нем в СБ (SH-1);

* **UC-3**: **Работа с материалами стажировки**:
    - все авторизированные участники могут просмотреть программу стажировок (SH-1, SH-2, SH-3, SH-4, SH-5);
    - стажер может отправлять домашнее задание (SH-5);
...
* **UC-4**: **Анонсы стажировок**:
    ...
    - любой может просмотреть анонс стажировоки (SH-1, SH-2, SH-3, SH-4, SH-5, SH-6);


* **UC-5**: **Управление стажировкой**:
...
* **UC-7**: **Управление стажерами и менторами**:
    ...
    - Руководитель стажировки имеет функцию поиска стажера, который уже присутствует в системе, и возможность назначения его на свою стажировку (SH-4);
...
* **QA-3**: **производительность**
    ...
    - время поиска пользователей составляет несколько секунд?;
